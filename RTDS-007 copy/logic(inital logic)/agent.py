...
class OpenStackAgent:
    ...
    def _parse_user_query_with_ai(self, user_query: str) -> Optional[Dict[str, Any]]:
        ...
        if not self.model:
            ...
            if "list networks" in user_query.lower() or "list all the networks" in user_query.lower() or "list my networks" in user_query.lower(): return {"function_name": "list_networks", "parameters": {}}
    ...
    def execute_command(self, user_query: str):
        ...
        if function_name == 'create_volume':
            ...
            if isinstance(provided_params['size_gb'], str):
                try:
                    # Strip any non-numeric characters from size_gb
                    size_str = ''.join(filter(str.isdigit, str(provided_params['size_gb'])))
                    if not size_str:
                        raise ValueError("Volume size must contain numeric value")
                    provided_params['size_gb'] = int(size_str)
